...
- Two new independently-toggleable map layers, **Zones** and **Labels**, alongside the existing Trails/Vectors/Uncertainty/Sensors toggles: Zones controls whether restricted/monitoring zone polygons are drawn at all, Labels controls a new always-on zone-name tooltip on top of them (distinct from the existing click-to-open popup).
- Notification drawer overhaul: each notification now has a real severity color and an explicit lifecycle state (Unread/Read/Acknowledged/Resolved, `notifState()`) -- for an incident-linked notification this is read straight from that incident's real `status` rather than tracked separately, so it can never disagree with the Alerts panel. An Acknowledge button drives the same real `POST /api/incidents/{id}/acknowledge` endpoint; a View track button jumps to the track if it still exists.
- Track list group headers (Friendly/Drone/Aircraft/...) now persist their collapsed/expanded state to `localStorage`, so a group an operator collapsed to focus on unknown activity stays collapsed across a full page reload, not just across poll cycles. Verified tracks also show a small badge with their corroborating sensor-type count.
- The details panel's top section is now explicitly ordered Identity -> Telemetry -> Action (previously Action sat above Identity/Telemetry) to match "identify the object, understand its movement, then decide what to do."
- `POST /api/tracks/{id}/verify-visual`: a human operator's structured visual-verification judgment (confirmed / a different object / a false detection / unable to determine, plus an optional note), surfaced as a "Visual verification" control under the Live view tab. Recorded to the audit log only (visible on the Timeline tab for admins) -- never touches `Track.classification` or `risk_score`, and deliberately not the fake PTZ auto-tracking confirmation this app has no closed-loop visual tracking to back.
- `IncidentStatus.INVESTIGATING` and `POST /api/incidents/{id}/investigate`: a real, server-enforced status between acknowledged and resolved (only reachable from acknowledged), surfaced as a "Start investigation" button in the Alerts panel. Alert rows now also show `acknowledged_by` and a working link to the related track.
- Denser track rows: the meta line now shows distance to the nearest zone, altitude, and heading (previously just raw latitude/longitude), plus a RISK badge once the score is actually high enough to be worth calling out (score < 5 shows no badge). The map's per-track popup card similarly now also shows classification confidence and zone status.
